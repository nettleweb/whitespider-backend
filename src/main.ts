...
class Session {
	...
	#focused: number = -1;
	get #focusedPage(): puppeteer.Page { return this.#pages[this.#focused]; }
	set #focusedPage(page: puppeteer.Page) { this.#pages[this.#focused] = page; }

	constructor(socket: Socket, context: puppeteer.BrowserContext, width: number, height: number, touch: boolean, landscape: boolean) {
		this.socket = socket;
	...
	async #updatePageSettings(page: puppeteer.Page) {
	...
	#checkRewriteURL(url: URL): string {
		switch (url.protocol) {
			case "http:":
			case "https:":
			case "data:":
	...
	readonly screenBuffer: () => Promise<string> = () => this.#focusedPage.screenshot({
		type: "jpeg",
		quality: 80,
		encoding: "base64",
		fullPage: false,
	...
	async end() {
		...
		(this as any).#context =
			...
			(this as any).#height =
			(this as any).#touch =
			(this as any).#landscape =
			(this as any).#focused = void 0;
	}
...
function initializeSession(session: Session) {
	const { socket } = session;

	socket.on("navigate", (url: string) => session.navigate(url));
	socket.on("event", (e: Event) => session.dispatchEvent(e));
